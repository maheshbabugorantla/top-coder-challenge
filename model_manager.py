...
def train_optimized_model():
    ...
    param_grids = {
        ...
        'xgb_fixed': {},
        'xgb_tuned': {'learning_rate': [0.1], 'max_depth': [4], 'n_estimators': [100]}
    }
    ...
    if kfold is None:
        ...
        for model_key, model_instance in models_config.items():
            ...
            if current_param_grid:
                grid_search = GridSearchCV(model_instance, current_param_grid, cv=kfold, scoring='neg_mean_absolute_error', n_jobs=1, verbose=0) # n_jobs=1
                grid_search.fit(X, y)
                ...
                cv_scores_array = cross_val_score(model_instance, X, y, cv=kfold, scoring='neg_mean_absolute_error', n_jobs=1) # n_jobs=1
    ...
    early_stopping_rounds_final = None
    if isinstance(best_model_obj, XGBRegressor):
        potential_esr = best_params_val.get('early_stopping_rounds') if best_params_val else None # best_params_val might be None for xgb_fixed
        if potential_esr is None:
            potential_esr = best_model_obj.get_params().get('early_stopping_rounds')
        if isinstance(potential_esr, list):
            early_stopping_rounds_final = potential_esr[0] if potential_esr else None
        else: early_stopping_rounds_final = potential_esr

    if early_stopping_rounds_final and isinstance(early_stopping_rounds_final, int) and early_stopping_rounds_final > 0:
        print(f"\nRefitting best model {best_model_name_val} with early stopping (rounds={early_stopping_rounds_final})...")
        # Ensure X has enough samples for a split; test_size=0.15 means at least 2 samples for X_eval_f, so X needs at least ~7 samples.
        # If X has fewer than, say, 7 samples, X_eval_f could be empty or too small.
        if X.shape[0] < 7: # Heuristic threshold for a meaningful split for early stopping.
             print(f"Dataset too small ({X.shape[0]} samples) for final eval split. Fitting on all data without early stopping refit.")
             best_model_obj.fit(X,y)
        else:
            X_train_f, X_eval_f, y_train_f, y_eval_f = train_test_split(X, y, test_size=0.15, random_state=42)
            if X_train_f.empty or X_eval_f.empty: # Should be caught by X.shape[0] < 7, but as a safeguard
                print("Train or Eval split resulted in empty data. Fitting on all data.")
                best_model_obj.fit(X, y)
            else:
                params_for_clone = best_model_obj.get_params()
                if 'early_stopping_rounds' in params_for_clone: del params_for_clone['early_stopping_rounds']
                final_model_for_refit = best_model_obj.__class__(**params_for_clone)
                final_model_for_refit.fit(X_train_f, y_train_f, early_stopping_rounds=early_stopping_rounds_final,
                                          eval_set=[(X_eval_f, y_eval_f)], verbose=False)
                best_model_obj = final_model_for_refit
            print("Final model refit with early stopping consideration complete.")
    else:
        print(f"\nFitting best model {best_model_name_val} on all training data...")
        best_model_obj.fit(X, y)
        print("Standard final model fit complete.")
...
def main():
    ...
    try:
        import numpy as numpy_module; np = numpy_module
        import pandas as pandas_module; pd = pandas_module
        import m2cgen as m2c_module; m2c = m2c_module
        from xgboost import XGBRegressor as XGBR_module; XGBRegressor = XGBR_module
        from sklearn.ensemble import RandomForestRegressor as RFR_module; RandomForestRegressor = RFR_module
        from sklearn.model_selection import KFold as KF_module, GridSearchCV as GSCV_module, cross_val_score as CVS_module, train_test_split as TTS_module
        KFold, GridSearchCV, cross_val_score, train_test_split = KF_module, GSCV_module, CVS_module, TTS_module
        from sklearn.metrics import mean_squared_error as MSE_module, mean_absolute_error as MAE_module
        mean_squared_error, mean_absolute_error = MSE_module, MAE_module
        import joblib as joblib_module; joblib = joblib_module
    ...
    if args.action == 'status':
        ...
        else: print("❌ Failed to delete model")
        ...
        else: print("❌ No saved model found. Run 'train' to create one.")
